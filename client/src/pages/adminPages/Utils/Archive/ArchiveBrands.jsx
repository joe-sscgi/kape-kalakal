...
export const loader = async ({ request }) => {
  try {
    ...
    const { data } = await customFetch.get(
      `/admin/utilities/archive-brands?${queryParams.toString()}`
    );
    return data;
  } catch (error) {
    console.error(error);
  ...
  try {
    if (data.action == "restore") {
      await customFetch.patch(
        `/admin/utilities/archive-brands/${data.ArchiveID}`,
        data
      );
        ...
        <Form method="post" className="form-action">
          <Modal.Body>
            <div className="form-action-container">
              <p>are you sure you want to this item?</p>
              <br />
              <input
                type="text"
                id="action"
