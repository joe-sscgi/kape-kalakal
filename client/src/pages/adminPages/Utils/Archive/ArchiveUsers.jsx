...
export const loader = async ({ request }) => {
  try {
    ...
    const sortBy = url.searchParams.get("sortBy") || "userUsername";
    ...
    const { data } = await customFetch.get(
      `/admin/utilities/archive-users?${queryParams.toString()}`
    );
    return data;
  } catch (error) {
    console.error(error);
  ...
  try {
    if (data.action == "restore") {
      await customFetch.patch(
        `/admin/utilities/archive-users/${data.ArchiveID}`,
        data
      );
...
const ArchiveUsers = () => {
  const { users, currentPage, totalPages } = useLoaderData();
  const list = Object.values(USER_TYPE);
        ...
        <div className="container">
          <div className="filters-container">
            ...
            <select
              ...
              className="form-control"
            >
              <option value="userUsername">Sort by Username</option>
              <option value="userEmail">Sort by Email</option>
              <option value="userUserType">Sort by Type</option>
            </select>
